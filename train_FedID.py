...
from model.model_UNet import UNet,ProtoLearning
...
if __name__ == '__main__':
    ...
    protoglob=ProtoLearning(num_classes=2, feature_dim=64)
    protoglob=protoglob.to(args.device)
    ...
    loss_npl_dict = {}
    global_prototypes = {
            "foreground": nn.Parameter(torch.randn(256).to(args.device)),#prototype_dim=256
            "background": nn.Parameter(torch.randn(256).to(args.device)),#prototype_dim=256
        }
    for i in range(4):
        IoU_npl_dict[i] = []
        loss_npl_dict[i]=[]
    ...
    for rnd in range(args.rounds):
        w_locals,proto_locals, loss_locals, dice_locals = [], [], [], []

        for idx in user_id:  # training over the subset
            

            local = trainer_locals[idx]  
            w_local,proto_local, loss_local,anRate = local.train_local(
            net=copy.deepcopy(netglob).to(args.device),proto=copy.deepcopy(protoglob).to(args.device), writer=writer,round=rnd,correct=correct[idx],thresh=thre[idx],corrN=correctNew)
            ...
            proto_locals.append(copy.deepcopy(proto_local))
            ...
            loss_npl_dict[idx].append(loss_locals[idx])
            truncated_losses = {client_id: losses[:rnd] for client_id, losses in loss_npl_dict.items()}
            ...
            if rnd==30:
                ...
                print(thre[idx])
            if rnd>30 and correct[idx]==0 and flag[idx]!=1:
                ...
                if line-IoU_npl_dict[idx][rnd]>0.03:
                    ...
                    if flag[0]==0 and flag[1]==0 and flag[2]==0 and flag[3]==0:
                        vote = []
                        for i in range(9):
                            gmm = GaussianMixture(n_components=2, random_state=i).fit(np.array(list(truncated_losses.values())))
                            gmm_pred = gmm.predict(np.array(list(truncated_losses.values())))
                        
                            noisy_clients = np.where(gmm_pred == np.argmax(gmm.means_.sum(1)))[0]
                            noisy_clients = set(noisy_clients)
                    ...
                    if (correct[0]!=0 and correct[1]!=0 and correct[2]!=0 and correct[3]!=0):
                        correctNew=rnd+21
        ...
        loss_len=[math.exp(1.0/loss_locals[0]),math.exp(1.0/loss_locals[1]),math.exp(1.0/loss_locals[2]),math.exp(1.0/loss_locals[3])]
        if rnd<=30  or rnd>=correctNew:
            dict_len = [626,591,570,646]
            proto_glob_fl=FedAvg(proto_locals, loss_len)
            ...
            proto_glob_fl=FedAvg(proto_locals, dict_len)
        # else:
        ...
        protoglob.load_state_dict(copy.deepcopy(proto_glob_fl))
