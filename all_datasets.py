    ...
    def __init__(self, root, list_path, flag):
        ...
        for item in self.img_ids:
            ...
            if flag==None:
                ...
                ori_label=osp.join(self.root,'label/',name)
            elif flag==0:
                label_file = osp.join(self.root,'2DlabelSingle/',name)###这个要改
                ori_label=osp.join(self.root,'2DlabelSingle/',name)
            elif flag==1:
                label_file = osp.join(self.root,'2DlabelSingle/',name)###这个要改
                ori_label=osp.join(self.root,'2DlabelSingle/',name)
            elif flag==2:
...
#需不需要一个gt
    ...
    def __getitem__(self, index):
        ...
        orilabel = Image.open(datafiles["ori"])
        orilabel = np.array(orilabel)
        orilabel = orilabel[np.newaxis, :]
...
#不能把原始数据变了哇
        ...
        return image, label, orilabel,name,index
